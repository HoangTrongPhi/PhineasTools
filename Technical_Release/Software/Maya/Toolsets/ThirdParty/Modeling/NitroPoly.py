...
# Update: 4.09pm   1/11/2024
# Done, đã fix lỗi not module pymel
# pymel chỉ support tối đa maya 2025 do đó maya2026 chưa sử dụng được
...
class NP():
    def __init__(self):
        ...
        self.hotkeysList = [{"name": "GrowLoop", "command": "import NitroPoly;NitroPoly.NP().growLoop()"},
                            ...
                            {"name": "UnFreezeTransform",
                             "command": "import NitroPoly;NitroPoly.NP().UnFreezeTranslate()"},
    ...
    def UI(self):
        ...
        def frameCollapseChanged(modelLayout):
            cmds.evalDeferred(
                "cmds.window('winModelingToolkit', e=1, h=sum([eval('cmds.' + cmds.objectTypeUI(child) + '(\\'' + child + '\\', q=1, h=1)') for child in cmds.columnLayout('" + modelLayout + "', q=1, ca=1)]))")

        ####################
        # Modiy selection
        ####################
        cmds.frameLayout(label="Modify Selection", w=160, cll=True, cl=0,
                         ec=partial(frameCollapseChanged, str(modelLayout)),
                         cc=partial(frameCollapseChanged, str(modelLayout)), bgc=frameColor)
        cmds.separator(h=1)
        ...
        #######################################
        cmds.frameLayout(label="Mesh and Uni-Connector / Uni-Remover", cll=True, cl=0,
                         ec=partial(frameCollapseChanged, str(modelLayout)),
                         cc=partial(frameCollapseChanged, str(modelLayout)), bgc=frameColor)
        cmds.separator(h=1)
        cmds.rowColumnLayout(numberOfColumns=4, columnSpacing=[(1, 8), (2, 2), (3, 5), (4, 2)])
        ...
        ##########################################
        cmds.frameLayout(label="Pivot and UnFreeze Transformations", cll=True, cl=0,
                         ec=partial(frameCollapseChanged, str(modelLayout)),
                         cc=partial(frameCollapseChanged, str(modelLayout)), bgc=frameColor)
        cmds.separator(h=1)
        cmds.rowColumnLayout(numberOfColumns=4, columnSpacing=[(1, 8), (2, 2), (3, 5), (4, 2)])
        ...
        # Topology Tools
        ################
        cmds.frameLayout(label="Topology Tools", cll=True, cl=0, ec=partial(frameCollapseChanged, str(modelLayout)),
                         cc=partial(frameCollapseChanged, str(modelLayout)), bgc=frameColor)
        cmds.separator(h=1)

        cmds.rowColumnLayout(numberOfColumns=2, columnSpacing=[(1, 8), (2, 5)])

        cmds.button(label='Corner Rotate 45 +', h=buttonHeight, w=winWidth / 2 - 15,
                    c=lambda *args: self.corner45(True), bgc=plusClr)
        cmds.button(label='Corner Rotate 45 -', h=buttonHeight, w=winWidth / 2 - 10,
                    c=lambda *args: self.corner45(False), bgc=minClr)
        cmds.setParent('..')
        cmds.separator(h=1)

        cmds.rowColumnLayout(numberOfColumns=3, columnSpacing=[(1, 8), (2, 5), (3, 5)])
        self.F2Threshold = cmds.floatSliderGrp(v=0.001, cw2=[40, 0], field=1, minValue=0.001, maxValue=1.0, step=0.001,
                                               w=40, h=10)
        cmds.button(label='F2 Extend', h=buttonHeight, w=winWidth - 65, c=lambda x: self.edgeExtend(), bgc=buttonColor)
        cmds.setParent('..')
        cmds.separator(h=1)
        cmds.rowColumnLayout(numberOfColumns=3, columnSpacing=[(1, 8), (2, 5), (3, 5)])
        self.BevelThreshold = cmds.floatSliderGrp(v=0.1, cw2=[40, 0], field=1, minValue=0.02, maxValue=1.0, step=0.01,
                                                  w=40, h=10)
        cmds.button(label="Bevel +", w=winWidth / 2 - 30, h=buttonHeight, c=lambda *args: self.bevelModifier(True),
                    bgc=plusClr)
        cmds.button(label="Bevel -", w=winWidth / 2 - 40, h=buttonHeight, c=lambda *args: self.bevelModifier(False),
                    bgc=minClr)
        cmds.setParent('..')
        cmds.separator(h=1)
        cmds.setParent('..')
        ...
        # Connect Tools
        ################
        cmds.frameLayout(label="Connect Tools", w=180, cll=True, cl=0,
                         ec=partial(frameCollapseChanged, str(modelLayout)),
                         cc=partial(frameCollapseChanged, str(modelLayout)), bgc=frameColor)
        cmds.separator(h=1)

        cmds.rowColumnLayout(numberOfColumns=4, columnSpacing=[(1, 8), (2, 5), (3, 5), (4, 5)])
        self.StitchThreshold = cmds.floatSliderGrp(v=0.1, cw2=[40, 0], field=1, minValue=0.02, maxValue=1.0, step=0.02,
                                                   w=40, h=10)
        ...
        # Loop Tools
        #############
        cmds.frameLayout(label="Loop Tools - Iterable : Click repeatedly for better result", cll=True, cl=0,
                         ec=partial(frameCollapseChanged, str(modelLayout)),
                         cc=partial(frameCollapseChanged, str(modelLayout)), bgc=frameColor)
        cmds.separator(h=1)
        ...
        # About
        #########
        cmds.frameLayout(label="Info", w=180, cll=0, cl=0, ec=partial(frameCollapseChanged, str(modelLayout)),
                         cc=partial(frameCollapseChanged, str(modelLayout)), bgc=frameColor)
        cmds.rowColumnLayout(numberOfColumns=1, columnSpacing=[(1, 0)])
        cmds.button(label=' Click here for Tool Help and Features', h=25, w=winWidth, c=lambda x: self.abt(),
                    bgc=devClr)
        cmds.setParent('..')
        cmds.setParent('..')
    ...
    def inLineMessage(self, message, fadeTime=3):
        pm.inViewMessage(amg='Error : <hl style=\"color:#FF0000;\">' + message + '</hl>', pos='midCenter', fade=True,
                         bkc=0X11550000, fit=fadeTime)
        pm.warning(message)
    ...
    def detatchClean(self):
        ...
        if faces:
            ...
            ii = 0
            newFaceDelete = []
            cmds.progressWindow(title='Detaching - Please Wait', progress=0, max=cnt, status='Processing...',
    ...
    def UnFreezeTranslate(self):
        ...
        for sel in selection:
            ...
            try:
                pos = [pm.getAttr(sel + ".translateX") * -1, pm.getAttr(sel + ".translateY") * -1,
                       cmds.getAttr(sel + ".translateZ") * -1]
            except:
                pm.delete(ch=1)
    ...
    def corner45(self, dir, *args):
        ...
        for s in sel:
            if ".e[" in str(s):
                ...
                fc.append(s)
        # Error Checking
        if len(ed) != 1 or len(fc) < 1: return self.inLineMessage(
            "Adjacent face(s) and single edge should be selected \n\n <i style=\"color:#00FF00;\">Use Multi selection mode</i>.")
    ...
    def cornerConnect(self):
        ...
        if chkEdge != None:
            ...
            if crnrEdges:
                ...
                cmds.progressWindow(title='Corner Connect', progress=0, max=len(crnrVerts),
                                    status='Connecting Edges...', isInterruptable=True)
    ...
    def cutAndStitch(self):
        ...
        if weld:
            ...
            newVerts = cmds.ls(
                str(Low[0].split(".")[0]) + '.vtx[' + str(totalVerts) + ":" + str(pm.polyEvaluate(v=1)) + ']', fl=1)
    ...
    def vertexToEdge(self):
        ...
        if ".vtx[" not in Vert or ".e[" not in Edge: return self.inLineMessage(
            "Select one Vertex and one Edge. \n\n <i style=\"color:#00FF00;\">Use Multi selection mode</i>.")
    ...
    def spaceloop(self):
        ...
        for single in edgeGroup:
            ...
            for index, vert in enumerate(verts):
                vertData[index] = {'vertex': vert, 'vertexPos': cmds.xform(vert, q=1, ws=1, t=1), 'cv': None,
                                   'cvPos': None}
